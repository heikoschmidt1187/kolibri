...
BatteryMonitor::BatteryMonitor(const uint8_t voltageMeasurementPin,
    ...
    : voltageMeasurementPin(voltageMeasurementPin),
      currentMeasurementPin(currentMeasurementPin), consumedCapacity(0.F),
...
BatteryMonitor::BatteryLevel BatteryMonitor::Init()
{
    ...
    if (voltage > 8.3F)
        capacityAtStart = TOTAL_BATTERY_CAPACITY; // battery considered to be full
    else
...
BatteryMonitor::BatteryLevel BatteryMonitor::Process()
{
    ...
    consumedCapacity = current * 1000.F * cycleTimeS / 3600.F + consumedCapacity;
    auto remaining =
        (capacityAtStart - consumedCapacity) / TOTAL_BATTERY_CAPACITY * 100;

    if (statusLogCycleCtr == 0U)
    {
        Serial.printf("Measured voltage: %f V | Measured current: %f A | Remaining capacity: %f %\n",
